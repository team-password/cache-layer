...
func (m MemoryCacheHandler) StoreAll(keyValues ...cachelayer.KeyValue) (err error) {
...
func (m MemoryCacheHandler) GetAll(keys schemas.PK) (data []cachelayer.ReturnKeyValue, err error) {
	returnKeyValues := make([]cachelayer.ReturnKeyValue, 0)
	for _, key := range keys {
		bytes, has := m.data[key]
		returnKeyValues = append(returnKeyValues, cachelayer.ReturnKeyValue{
			KeyValue: cachelayer.KeyValue{
				Key:   key,
				Value: bytes,
			},
			Has: has,
...
func NewGormCacheHandler() *cachelayer.CacheHandler {
	return cachelayer.NewCacheHandler(NewMemoryCacheHandler(), NewGormDd())
}

// NewGormDd Create a gorm db
func NewGormDd() cachelayer.IDB {
	db, err := gorm.Open(mysql.Open("root:123456@tcp(127.0.0.1:3306)/team-password?charset=utf8&parseTime=True&loc=Local"), &gorm.Config{})
	if err != nil {
		panic(err)
	}
	return GormDB{
